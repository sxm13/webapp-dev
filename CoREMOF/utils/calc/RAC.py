...
def get(struc):
    ...
    os.makedirs("./data/tmp_files/tmp_rac", exist_ok=True)
    
    full_names, full_descriptors = get_MOF_descriptors(
                                                        struc,
                                                        3,
                                                        path="./data/tmp_files/tmp_rac",
                                                        xyzpath=f'./data/tmp_files/tmp_rac/tmp.xyz',
                                                        max_num_atoms=6000
                                                        )
    ...
    for metal in metal_fnames:
        result_rac["Metal"][metal] = descriptor_data[metal]
    for linker in linker_fnames:
        result_rac["Linker"][linker] = descriptor_data[linker]
    for fg in fg_fnames:
        result_rac["Function-group"][fg] = descriptor_data[fg]

    remove_dir_with_permissions("./data/tmp_files/tmp_rac")
